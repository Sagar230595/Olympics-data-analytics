...
def country_event_heatmap(df, country):
    ...
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = temp_df[temp_df['region'] == country]
    pt = new_df.pivot_table(values='Medal',index='Sport',columns='Year',aggfunc='count').fillna(0)
    return pt


def most_succesful_athlete(df, country):
    temp_df = df.dropna(subset=['Medal'])
...
def men_vs_women(df):
    ...
    total = men.merge(women, on='Year')
    total.rename(columns={'Name_x': 'Male', 'Name_y': 'Female'}, inplace=True)
